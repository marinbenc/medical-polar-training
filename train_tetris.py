...
def add_shape_priors(batch, device='cuda', non_blocking=True):
  x, y = batch

  # make prior as a circle
  prior = np.zeros(x.shape[2:])
  r = 50
  center = (x.shape[-1] // 2, x.shape[-2] // 2)
  prior = cv.circle(prior, center, radius=r, color=255, thickness=-1) / 255.0
  prior = torch.from_numpy(prior)

  # one prior per image in batch
  priors = prior[(None,)*2]
  priors = priors.repeat(x.shape[0], 1, 1, 1)

  x = torch.cat((x, priors), 1)
  
  return (
    convert_tensor(x, device=device, non_blocking=non_blocking).float(),
...
def main(args):
    ...
    trainer = create_supervised_trainer(model, optimizer, criterion, device=device, prepare_batch=add_shape_priors)
    trainer.logger = setup_logger('Trainer')

    train_evaluator = create_supervised_evaluator(model, metrics=metrics, device=device, prepare_batch=add_shape_priors)
    train_evaluator.logger = setup_logger('Train Evaluator')
    validation_evaluator = create_supervised_evaluator(model, metrics=metrics, device=device, prepare_batch=add_shape_priors)
